...
public class SubscriptionRepositoryJpaImpl implements SubscriptionRepository {
    ...
    @Override
    public List<SubscriptionDVO> getSubscriptionByGroup(GroupDVO group,
	    SubscriptionDVO.State aState) {
	Query q = entityManager
		.createQuery("select ug from SubscriptionDVO ug where ug.groupId = :groupId and ug.state = :state");
	...
	Query q = entityManager
		.createQuery("select s from SubscriptionDVO s where s.groupId = :groupId and s.userId = :userId");
	...
	if (subscriptionDVOs == null || subscriptionDVOs.isEmpty()) {
	    return null;
	} else {
    ...
    @Override
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
    public void updateState(SubscriptionDVO subscriptionDVO,
	    SubscriptionDVO.State state) {
	Query q = entityManager
		.createQuery("update SubscriptionDVO s set s.state = :state where s.groupId = :groupId and s.userId = :userId");
	...
	Query q = entityManager
		.createQuery("delete from SubscriptionDVO s where s.groupId = :groupId and s.userId = :userId");
